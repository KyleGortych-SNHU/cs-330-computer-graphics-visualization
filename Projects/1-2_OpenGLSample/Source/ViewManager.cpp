...
GLFWwindow* ViewManager::CreateDisplayWindow(const char* windowTitle)
{
  ...
  // initialize mouse position to center of framebuffer
  gLastX = framebufferWidth / 2.0f;
  gLastY = framebufferHeight / 2.0f;
...
void ViewManager::PrepareSceneView() {
  ...
  // use framebuffer size instead of fixed window
  ...
  if(m_pWindow != nullptr) {
    ...
    if (framebufferWidth > framebufferHeight) {
        scale = (double)framebufferHeight / (double)framebufferWidth;
        projection = glm::ortho(-5.0f, 5.0f,
                                -5.0f * (float)scale, 5.0f * (float)scale,
                                0.1f, 100.0f);
    } else if (framebufferWidth < framebufferHeight) {
        scale = (double)framebufferWidth / (double)framebufferHeight;
        projection = glm::ortho(-5.0f * (float)scale, 5.0f * (float)scale,
                                -5.0f, 5.0f,
                                0.1f, 100.0f);
    } else {
      projection = glm::ortho(-5.0f, 5.0f, -5.0f, 5.0f, 0.1f, 100.0f);
